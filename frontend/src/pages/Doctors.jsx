...
const Doctors = () => {
  ...
  useEffect(() => {
    if (speciality) {
      const filterKey = normalize(speciality);
      setFilterDoc(
        doctors.filter((doc) => normalize(doc.speciality) === filterKey),
      );
    } else {
      ...
      <div className="flex flex-col sm:flex-row items-start gap-5 mt-5">
        ...
        >
          ...
          <p
            ...
            className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality === "Cardiologist" ? "bg-[#E2E5FF] text-black " : ""}`}
          >
            Cardiologist
          </p>
          <p
            onClick={() =>
              speciality === "Orthopedist"
                ? navigate("/doctors")
                : navigate("/doctors/Orthopedist")
            }
            className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality === "Orthopedist" ? "bg-[#E2E5FF] text-black " : ""}`}
          >
            Orthopedist
          </p>
        </div>
